...
import type { FineliState, Range, FineliReducerActionType } from '../../../types';
...
const RangeSlider = ({
  ...
  name: keyof FineliState;
  margin: number;
  dispatchType: FineliReducerActionType;
  type?: 'min' | 'max' | 'both';
}) => {
  ...
  // name must be correct for this to work!
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      ...
      dispatch({
        type: dispatchType,
        payload: { [name]: { min: Math.round(min), max: Math.round(max) } },
      });
    }, 200);
